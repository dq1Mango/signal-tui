...
use libsignal_service::{
  ...
  proto::{
    ...
    AttachmentPointer, DataMessage, EditMessage, GroupContextV2, NullMessage, SyncMessage, Verified,
  },
  ...
  push_service::{
    AccountAttributes, DeviceCapabilities, DeviceInfo, PushService, ServiceError, ServiceIds, WhoAmIResponse,
    DEFAULT_DEVICE_ID,
  },
...
impl Registered {
  ...
  pub(crate) async fn set_account_attributes<S: Store>(&mut self, store: &mut S) -> Result<(), Error<S::Error>> {
    trace!("setting account attributes");
    let mut account_manager = AccountManager::new(self.identified_push_service(), Some(self.data.profile_key));
...
impl<S: Store> Manager<S, Registered> {
  ...
  fn unidentified_push_service(&self) -> PushService {
    self
      ...
      .get_or_init(|| PushService::new(self.state.service_configuration(), None, crate::USER_AGENT))
      .clone()
  }
  ...
  async fn identified_websocket(&self, require_unused: bool) -> Result<SignalWebSocket, Error<S::Error>> {
      ...
      .filter(|ws| !(require_unused && ws.is_used()))
    {
      ...
      None => {
        ...
        let ws = self
          .identified_push_service()
          .ws("/v1/websocket/", "/v1/keepalive", headers, Some(self.credentials()))
          .await?;
  ...
  pub(crate) async fn register_pre_keys(&mut self) -> Result<(), Error<S::Error>> {
    ...
    let mut account_manager = AccountManager::new(self.identified_push_service(), Some(self.state.data.profile_key));

    let mut rng = rand::rng();

    account_manager
      .update_pre_key_bundle(&mut self.store.aci_protocol_store(), ServiceIdKind::Aci, true, &mut rng)
      .await?;

    account_manager
      .update_pre_key_bundle(&mut self.store.pni_protocol_store(), ServiceIdKind::Pni, true, &mut rng)
      .await?;
  ...
  pub async fn request_contacts(&mut self) -> Result<(), Error<S::Error>> {
    ...
    self
      .send_message(self.state.data.service_ids.aci(), sync_message, timestamp)
      .await?;
  ...
  async fn sender_certificate(&self) -> Result<SenderCertificate, Error<S::Error>> {
    ...
    let mut unidentified_sender_certificate = self.state.unidentified_sender_certificate.lock().await;
    if needs_renewal(unidentified_sender_certificate.as_ref()) {
      let sender_certificate = self
        .identified_push_service()
        .get_uuid_only_sender_certificate()
        .await?;
      self.store.save_sender_certificate(&sender_certificate).await?;
      unidentified_sender_certificate.replace(sender_certificate);
    }

    Ok(unidentified_sender_certificate.clone().expect("logic error"))
  }

  pub async fn submit_recaptcha_challenge(&self, token: &str, captcha: &str) -> Result<(), Error<S::Error>> {
    let mut account_manager = AccountManager::new(self.identified_push_service(), None);
    account_manager.submit_recaptcha_challenge(token, captcha).await?;
    Ok(())
  }
  ...
  pub async fn retrieve_profile(&mut self) -> Result<Profile, Error<S::Error>> {
    self
      .retrieve_profile_by_uuid(self.state.data.service_ids.aci, self.state.data.profile_key)
      .await
  }
  ...
  pub async fn retrieve_profile_by_uuid(
    ...
    if let Some(profile) = self.store.profile(aci.into(), profile_key).await.ok().flatten() {
      return Ok(profile);
    }

    let mut account_manager = AccountManager::new(self.identified_push_service(), Some(profile_key));

    let profile = account_manager.retrieve_profile(aci).await?;

    let _ = self.store.save_profile(aci.into(), profile_key, profile.clone()).await;
    Ok(profile)
  }

  pub async fn retrieve_group_avatar(&mut self, context: GroupContextV2) -> Result<Option<AvatarBytes>, Error<S::Error>> {
    ...
    // TODO: Is there some way to know if this is outdated?
    if let Some(avatar) = self.store.group_avatar(master_key_bytes).await.ok().flatten() {
      return Ok(Some(avatar));
    }

    let mut gm = self.groups_manager()?;
    let Some(group) = upsert_group(&self.store, &mut gm, context.master_key(), &context.revision()).await? else {
      return Ok(None);
    };
  ...
  pub async fn retrieve_profile_avatar_by_uuid(
    ...
    // TODO: Is there some way to know if this is outdated?
    if let Some(avatar) = self.store.profile_avatar(uuid, profile_key).await.ok().flatten() {
      return Ok(Some(avatar));
    }

    let profile = if let Some(profile) = self.store.profile(uuid, profile_key).await.ok().flatten() {
      profile
    } else {
    ...
    let _ = self.store.save_profile_avatar(uuid, profile_key, &avatar).await;
    Ok(Some(avatar))
  }
  ...
  pub async fn receive_messages(&mut self) -> Result<impl Stream<Item = Received>, Error<S::Error>> {
        ...
        match state.encrypted_messages.next().await {
          Some(Ok(Incoming::Envelope(envelope))) => {
            let envelope = {
              ...
              match ServiceId::parse_from_service_id_string(envelope.destination_service_id()) {
                None | Some(ServiceId::Aci(_)) => state.service_cipher_aci.open_envelope(envelope, &mut rng()).await,
                Some(ServiceId::Pni(pni)) => {
                  if pni == state.service_ids.pni() && envelope.source_service_id.is_none() {
              ...
              Ok(Some(content)) => {
                    ...
                    match operation.r#type() {
                      sticker_pack_operation::Type::Install => {
                        ...
                        tokio::spawn(async move {
                          match download_sticker_pack(store, push_service, &operation).await {
                            Ok(sticker_pack) => {
                              debug!(
                      ...
                      sticker_pack_operation::Type::Remove => {
                        match state.store.remove_sticker_pack(operation.pack_id()).await {
                          Ok(was_present) => {
                ...
                }) = &content.body
                {
                  ...
                  if let Ok(Some(group)) =
                    upsert_group(&state.store, &mut state.groups_manager, master_key_bytes, revision).await
                  {
                    trace!(?group, "upserted group");
                  }
                }

                if let Err(error) = save_message(&mut state.store, &mut state.push_service, content.clone(), None).await {
  ...
  pub async fn send_message_to_group(
    ...
    let master_key_bytes = master_key_bytes.try_into().expect("Master key bytes to be of size 32.");
    ...
    let Some(group) = upsert_group(&self.store, &mut groups_manager, &master_key_bytes, &0).await? else {
      ...
      .filter(|m| m.aci != self.state.data.service_ids.aci())
    {
      ...
      let include_pni_signature = false;
      recipients.push((member.aci.into(), unidentified_access, include_pni_signature));
    }
  ...
  pub async fn clear_sessions(&self, recipient: &ServiceId) -> Result<(), Error<S::Error>> {
    ...
    self.store.aci_protocol_store().delete_all_sessions(recipient).await?;
    self.store.pni_protocol_store().delete_all_sessions(recipient).await?;
    Ok(())
  }
  ...
  pub async fn get_attachment(&self, attachment_pointer: &AttachmentPointer) -> Result<Vec<u8>, Error<S::Error>> {
    ...
    if ciphertext.len() > DECRYPT_IN_THREAD_THRESHOLD {
      ciphertext = tokio::task::spawn_blocking(move || decrypt_in_place(key, &mut ciphertext).map(|_| ciphertext))
        .await
  ...
  pub async fn sticker_metadata(&mut self, pack_id: &[u8], sticker_id: u32) -> Result<Option<Sticker>, Error<S::Error>> {
    Ok(
      self
        .store
        .sticker_pack(pack_id)
        .await?
        .and_then(|pack| pack.manifest.stickers.iter().find(|&x| x.id == sticker_id).cloned()),
    )
  }
  ...
  pub async fn send_session_reset(&mut self, recipient: &ServiceId, timestamp: u64) -> Result<(), Error<S::Error>> {
  ...
  async fn new_message_sender(&self) -> Result<MessageSender<S::AciStore>, Error<S::Error>> {
    ...
    let pni_identity_keypair = self.store.pni_protocol_store().get_identity_key_pair().await?;
  ...
  fn new_service_cipher_aci(&self) -> ServiceCipher<S::AciStore> {
    ServiceCipher::new(
      ...
      self.state.service_configuration().unidentified_sender_trust_roots,
      self.state.data.service_ids.aci,
      self.state.device_id(),
    )
  }
  ...
  pub async fn unlink_secondary(&self, device_id: i64) -> Result<(), Error<S::Error>> {
    ...
    self.identified_push_service().unlink_device(device_id).await?;
    Ok(())
  }
...
async fn download_sticker_pack<C: ContentsStore>(
  ...
  let mut sticker_pack_manifest: StickerPackManifest = libsignal_service::proto::Pack::decode(ciphertext.as_slice())
...
async fn download_sticker<C: ContentsStore>(
  ...
  let mut sticker_stream = push_service.get_sticker(&hex::encode(pack_id), sticker_id).await?;
...
async fn save_message<S: Store>(
  ...
  let message = match message.body {
    ...
    ContentBody::DataMessage(ref data_message @ DataMessage { ref profile_key, .. })
    | ContentBody::SynchronizeMessage(SyncMessage {
      sent:
        Some(sync_message::Sent {
          message: Some(ref data_message @ DataMessage { ref profile_key, .. }),
          ..
        }),
      ..
    }) => {
      ...
      if let Some(profile_key_bytes) = profile_key.clone().and_then(|p| p.try_into().ok()) {
            ...
            .is_none_or(|p| p.bytes != profile_key.bytes)
        {
          if let Some(aci) = sender.aci() {
            ...
            let encrypted_profile = push_service.retrieve_profile_by_id(aci, Some(profile_key)).await?;
      ...
      if let Some(expire_timer) = data_message.expire_timer {
        ...
        store.update_expire_timer(&thread, expire_timer, version).await?;
      }
    ...
    ContentBody::SynchronizeMessage(msg) => {
      debug!(?msg, "skipping saving sync message without interesting fields");
      None
    }
