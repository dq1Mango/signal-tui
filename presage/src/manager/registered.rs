...
impl<S: Store> Manager<S, Registered> {
  ...
  pub async fn retrieve_group_avatar(
    ...
    let Some(group) = upsert_group(&self.store, &mut gm, context.master_key(), &context.revision()).await?
    else {
      return Ok(None);
    };
  ...
  async fn groups_manager(&self) -> Result<GroupsManager<InMemoryCredentialsCache>, Error<S::Error>> {
              ...
              let envelope = {
                ...
                match ServiceId::parse_from_service_id_string(envelope.destination_service_id()) {
                  None | Some(ServiceId::Aci(_)) => {
                    state.service_cipher_aci.open_envelope(envelope, &mut rng()).await
                  }
                    ...
                    match request.r#type() {
                      RequestType::Contacts => {
                        let contacts = state
                          ...
                          .contacts()
                          .await
                          .map(|i| i.collect::<Result<Vec<_>, _>>().unwrap_or_default())
                          .unwrap_or_default();
                        let result = state
                          .message_sender
                          .send_contact_details(
                            ...
                            contacts
                              .into_iter()
                              .map(|c| libsignal_service::sender::ContactDetails {
                                number: c.phone_number.map(|p| p.to_string()),
                                aci: Some(c.uuid.to_string()),
                                aci_binary: Some(c.uuid.into_bytes().into()),
                                name: Some(c.name),
                                avatar: c.avatar.map(|a| {
                                  libsignal_service::proto::contact_details::Avatar {
                    ...
                    for operation in sticker_pack_operation {
                      match operation.r#type() {
                        sticker_pack_operation::Type::Install => {
                          ...
                          let unidentified_websocket = state.unidentified_websocket.clone();
                          let operation = operation.clone();

                          // download stickers in the background
                          tokio::spawn(async move {
                            match download_sticker_pack(store, unidentified_websocket, &operation).await {
                              Ok(sticker_pack) => {
                                debug!(
  ...
  pub async fn send_message(
    ...
    save_message(&mut self.store, &mut identified_websocket, content, Some(thread)).await?;
  ...
  pub async fn get_attachment(
    ...
    if ciphertext.len() > DECRYPT_IN_THREAD_THRESHOLD {
      ciphertext =
        tokio::task::spawn_blocking(move || decrypt_in_place(key, &mut ciphertext).map(|_| ciphertext))
          .await
  ...
  pub async fn unlink_secondary(&self, device_id: impl TryInto<DeviceId>) -> Result<(), Error<S::Error>> {
...
async fn download_sticker_pack<C: ContentsStore>(
  ...
  for sticker in &mut sticker_pack_manifest.stickers {
    match download_sticker::<C>(&mut unidentified_websocket, pack_id, pack_key, sticker.id).await {
