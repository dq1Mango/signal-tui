...
use presage::{
  libsignal_service::{
    ...
    protocol::{
      CiphertextMessageType, DeviceId, Direction, GenericSignedPreKey, IdentityChange, IdentityKey, IdentityKeyPair,
      IdentityKeyStore, KyberPreKeyId, KyberPreKeyRecord, KyberPreKeyStore, PreKeyId, PreKeyRecord, PreKeyStore,
      ProtocolAddress, ProtocolStore, PublicKey, SenderKeyRecord, SenderKeyStore, ServiceId, SessionRecord, SessionStore,
...
impl PreKeyStore for SqliteProtocolStore {
  ...
  async fn remove_pre_key(&mut self, prekey_id: PreKeyId) -> Result<(), SignalProtocolError> {
    let id: u32 = prekey_id.into();
    query!("DELETE FROM pre_keys WHERE id = ? AND identity = ?", id, self.identity)
      .execute(&self.store.db)
      .await
      .into_protocol_error()?;
    Ok(())
...
impl PreKeysStore for SqliteProtocolStore {
  ...
  async fn kyber_pre_keys_count(&self, _last_resort: bool) -> Result<usize, SignalProtocolError> {
    query_scalar!("SELECT COUNT(id) FROM kyber_pre_keys WHERE identity = ?", self.identity)
      .fetch_one(&self.store.db)
      .await
      .into_protocol_error()
      .map(|count| count.try_into().expect("invalid usize"))
...
impl KyberPreKeyStoreExt for SqliteProtocolStore {
  ...
  async fn remove_kyber_pre_key(&mut self, kyber_prekey_id: KyberPreKeyId) -> Result<(), SignalProtocolError> {
    let id: u32 = kyber_prekey_id.into();
    query!(
...
impl IdentityKeyStore for SqliteProtocolStore {
  ...
  async fn get_local_registration_id(&self) -> Result<u32, SignalProtocolError> {
    let data =
      self.store.load_registration_data().await?.ok_or_else(|| {
        SignalProtocolError::InvalidState("failed to load registration ID", "no registration data".into())
      })?;
