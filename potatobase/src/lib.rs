...
impl SqliteStore {
  pub async fn open(url: &str, trust_new_identities: OnNewIdentity) -> Result<Self, SqliteStoreError> {
  ...
  pub async fn open_with_passphrase(
    ...
    match Self::open_with_options(options.clone(), trust_new_identities.clone()).await {
      ...
      Err(SqliteStoreError::Migrate(sqlx::migrate::MigrateError::Execute(sqlx::Error::Database(e))))
        if e.code().is_some_and(|c| c.as_ref() == "26") =>
      {
        ...
        let Some(passphrase) = passphrase else {
          return Err(SqliteStoreError::Migrate(sqlx::migrate::MigrateError::Execute(
  ...
  async fn open_migrate_to_encrypted(
    ...
    let no_password_options = no_password_options.create_if_missing(true).foreign_keys(true);
    let no_password_db = Self::open_with_options(no_password_options, trust_new_identities.clone()).await?;
...
impl StateStore for SqliteStore {
  ...
  async fn set_aci_identity_key_pair(
    ...
    query!("INSERT OR REPLACE INTO kv (key, value) VALUES (?, ?)", key, value)
      .execute(&self.db)
      .await?;
    Ok(())
  }
